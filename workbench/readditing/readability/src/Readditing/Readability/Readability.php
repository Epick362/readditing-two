<?php 
use Readditing\Readability\JSLikeHTMLElement as JSLikeHTMLElement;
	...
	public function init()
	{
		...
		if ($bodyElems->length > 0) {
			if ($this->bodyCache == null) {
				$this->bodyCache = $this->DOMinnerHTML($bodyElems->item(0));
			}
			if ($this->body == null) {
	...
	protected function grabArticle($page=null) {
		...
		**/
		...
		//for ($nodeIndex=$targetList->length-1; $nodeIndex >= 0; $nodeIndex--) {
			...
			if ($tagName == 'DIV') {
				if (!preg_match($this->regexps['divToPElements'], $this->DOMinnerHTML($node))) {
					//$this->dbg('Altering div to p');
					$newNode = $this->dom->createElement('p');
					try {
						$newNode->innerHTML = $this->DOMinnerHTML($node);
